...
def switch_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    split_l=[(word[:i], word[i:]) for i in range(len(word))]
    switch_l=[L+R[1:2]+R[0:1]+R[2:] for L, R in split_l if len(R)-1]
    ### END CODE HERE ###
...
def replace_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    split_l=[(word[:i], word[i:]) for i in range(len(word))]
    replace_l=[a[:2]+c+b[1:] for a, b in split_l if len(b) for c in letters ]
...
def insert_letter(word, verbose=False):
    ...
    insert_l=[a+c+b[0:] for a,b in split_l if len(b)+1 for c in letters]
    ### END CODE HERE ###
...
def edit_one_letter(word, allow_switches = True):
    ...
    ### START CODE HERE ###
    edit_one_set=set(switch_letter(word)+delete_letter(word)+insert_letter(word)+replace_letter(word)) if allow_switches else set(delete_letter(word)+insert_letter(word)+replace_letter(word))
    ### END CODE HERE ###
...
def edit_two_letters(word, allow_switches = True):
    ...
    for w in edit_one_set:
        edit_two_set=edit_two_set.union(edit_one_letter(w,allow_switches))
    ### END CODE HERE ###
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    ### START CODE HERE ###
    best_words=Counter()
    a_set=edit_one_letter(word,verbose)
    b_set=edit_two_letters(word,verbose)
    if word in vocab: 
        suggestions.append(word)
    elif a_set.intersection(vocab):
        suggestions.extend(list(a_set.intersection(vocab)))  
    elif b_set.intersection(vocab):    
        suggestions.extend(list(b_set.intersection(vocab)))
    else:
        suggestions.append(word)
